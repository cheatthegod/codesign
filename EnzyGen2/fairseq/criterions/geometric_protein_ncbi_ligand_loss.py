...
@dataclass
class GeometricProteinNCBISubstrateConfig(FairseqDataclass):
    tpu: bool = II("common.tpu")
    encoder_factor: float = field(
        default=1.0,
        metadata={"help": "the importance of the sequence recovery loss"}
    )
    decoder_factor: float = field(
        default=1.0,
        metadata={"help": "the importance of the structure prediction loss"}
    )
    binding_factor: float = field(
        default=1.0,
        metadata={"help": "the importance of the protein-ligand binding prediction loss"}
    )


@register_criterion("geometric_protein_ncbi_substrate_loss", dataclass=GeometricProteinNCBISubstrateConfig)
class GeometricProteinNCBISubstrateLoss(FairseqCriterion):
    """
    Loss: sequence recovery loss, structure prediction loss and protein-ligand binding prediction loss
    """

    def __init__(self, cfg: GeometricProteinNCBISubstrateConfig, task):
        super().__init__(task)
        self.tpu = cfg.tpu
    ...
    def forward(self, model, sample, reduce=True):
        ...
        raw_target_coor = target_input["target_coor"]
        # Check for NaN or inf values in target_input
        if not torch.isfinite(raw_target_coor).all():
            nan_inf_mask = ~torch.isfinite(raw_target_coor)
            nan_inf_indices = torch.nonzero(nan_inf_mask)
            print(f"NaN/inf indices in target_coor: {nan_inf_indices}")
            print(f"Number of NaN/inf values: {nan_inf_mask.sum().item()}")

        if 'ligand_input' in sample:
            ligand_input = sample["ligand_input"]
            encoder_out, decoder_out, scores = model(source_input["src_tokens"],
                                                    source_input["src_lengths"],
                                                    target_coor,
                                                    motif, ncbi,
                                                    ligand_input["ligand_coor"], ligand_input["ligand_atom"])
        else:
        ...
        if 'ligand_input' in sample:
            # binding loss
            labels = ligand_input["ligand_binding"]
        ...
        if torch.isfinite(loss_decoder):
            ...
            loss = self.encoder_factor * loss_encoder + self.binding_factor * loss_binding
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        metrics.log_scalar(
            "loss", loss / sample_size / 8, round=3
        )
        metrics.log_scalar(
            "sequence loss", loss_encoder / sample_size / math.log(2) / 8, sample_size, round=3
        )
        metrics.log_scalar(
            "coordinate loss", loss_decoder / sample_size / 8, sample_size, round=3
        )
        metrics.log_scalar(
            "binding loss", loss_binding / total_batch / 8, sample_size, round=3
        )
        metrics.log_scalar(
