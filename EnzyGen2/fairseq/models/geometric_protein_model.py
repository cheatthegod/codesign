...
def resolve_runtime_device(args):
    return torch.device(
        "cpu" if getattr(args, "cpu", False) else ("cuda" if torch.cuda.is_available() else "cpu")
    )
...
class GeometricProteinNCBIModel(TransformerModel):
    ...
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        ...
        decoder = EGNN(in_node_nf=args.encoder_embed_dim, hidden_nf=args.encoder_embed_dim, out_node_nf=3,
                       in_edge_nf=0, device=runtime_device, n_layers=args.decoder_layers, attention=True, mode=args.egnn_mode)
        return decoder

    def forward(self, src_tokens, src_lengths, coors, motifs, ncbi):
...
class GeometricProteinNCBISubstrateModel(GeometricProteinNCBIModel):
    def __init__(self, args, encoder, decoder):
        ...
        self.substrate_egnn = SubstrateEGNN(in_node_nf=args.encoder_embed_dim, hidden_nf=args.encoder_embed_dim,
                                                out_node_nf=3, in_edge_nf=0, device=runtime_device, n_layers=3, attention=True,
                                                mode=args.egnn_mode)
        self.score = nn.Linear(args.encoder_embed_dim * 2, 2)

    def forward(self, src_tokens, src_lengths, coors, motifs, ncbi, substrate_coor=None, substrate_atom=None):
        need_head_weights = False
        return_contacts = False
        if return_contacts:
            need_head_weights = True
...
@register_model_architecture("geometric_protein_model_ncbi", "geometric_protein_model_ncbi")
def base_architecture(args):
    transformer_base_architecture(args)
    

@register_model_architecture("geometric_protein_model_ncbi", "geometric_protein_model_ncbi_esm")
def transformer_ncbi_esm(args):
...
@register_model_architecture("geometric_protein_model_ncbi_substrate", "geometric_protein_model_ncbi_substrate_esm")
def transformer_ncbi_esm_ligand(args):
    args.encoder_embed_dim = getattr(args, "encoder_embed_dim", 320)
    args.encoder_ffn_embed_dim = getattr(args, "encoder_ffn_embed_dim", 1280)
    args.encoder_attention_heads = getattr(args, "encoder_attention_heads", 8)
    args.encoder_layers = getattr(args, "encoder_layers", 6)
