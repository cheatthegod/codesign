...
def main(cfg: DictConfig, override_args=None):
    ...
    topp_probability = cfg.task.topp_probability
    ...
    for subset in cfg.dataset.valid_subset.split(","):
        try:
            task.load_dataset(subset, combine=False, epoch=1, data_stage=data_stage, task_cfg=saved_cfg.task)
        ...
        for i, sample in enumerate(progress):
            ...
            line = " ".join(str(item.item()) for item in _probs[0])
            fw_ll.write(line + "\n")
            fw_ll.flush()
        ...
        print(task.cfg.protein_task)
        print("coordinate loss: {}".format(np.mean(np.nan_to_num(rmsd, nan=0))))
