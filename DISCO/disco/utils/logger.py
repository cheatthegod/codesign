...
class WarningFilter(logging.Filter):
    """Logging filter that suppresses specific warning messages (e.g., simtk.openmm)."""

    def filter(self, record):
        """Determines whether the given log record should be emitted.

        Args:
            record: The ``logging.LogRecord`` to evaluate.

        Returns:
            False if the record message contains a suppressed pattern,
            True otherwise.
        """
