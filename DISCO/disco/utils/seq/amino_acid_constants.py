...
element_types = [
    atom_type if atom_type == "ATM" else atom_type[0] for atom_type in atom_types
]
atom_types_set = set(atom_types)
atom_order = {atom_type: i for i, atom_type in enumerate(atom_types)}
atom_type_num = len(atom_types)  # := 37 + 10 null types := 47.
res_rep_atom_index = (
    1  # The index of the atom used to represent the center of the residue.
)
...
BIOMOLECULE_CHAIN = "polypeptide(L)"
...
def _make_constants():
    ...
    for restype, restype_letter in enumerate(restype_1to3.keys()):
        ...
        for compact_atomidx, atomname in enumerate(
            restype_name_to_compact_atom_names[resname]
        ):
            if not atomname:
                continue
