...
def to_device(obj, device):
    ...
    Args:
        obj: A ``torch.Tensor`` or a (possibly nested) dict of tensors.
        device: The target device (e.g., ``"cpu"``, ``"cuda:0"``).

    Returns:
        The input object with all tensors moved to ``device``.

    Raises:
        Exception: If ``obj`` is not a ``torch.Tensor`` or ``dict``.
    """
...
def cdist(a: torch.Tensor, b: torch.Tensor = None):
    ...
    Uses ``torch.cdist`` with ``donot_use_mm_for_euclid_dist`` mode for
    improved numerical accuracy.

    Args:
        a: Tensor of shape ``(..., M, D)``.
        b: Tensor of shape ``(..., N, D)``. If None, distances are computed
            between ``a`` and itself.

    Returns:
        Tensor of shape ``(..., M, N)`` containing pairwise distances.
    """
