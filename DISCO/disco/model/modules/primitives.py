...
class AdaptiveLayerNorm(nn.Module):
    ...
    def zero_init(self):
        """Initializes adaptive layer norm weights and biases to zero."""
...
def rearrange_qk_to_dense_trunk(
    q: torch.Tensor | list[torch.Tensor],
...
def gather_pair_embedding_in_dense_trunk(
    ...
    idx_q: torch.Tensor,
    idx_k: torch.Tensor,
    broadcast_atom_to_token_idx: bool = False,
):
    """Gathers pair embeddings into dense trunk windows using query/key index maps.

    Selectively gathers elements from a pair embedding tensor using two sets of
    indices, where all inputs have a leading batch dimension.

    Args:
        x (torch.Tensor): Pair embedding tensor.
            [batch_size, N_token, N_token, d]
        idx_q (torch.Tensor): Query indices for gathering.
            [batch_size, N_b, N_q]
        idx_k (torch.Tensor): Key indices for gathering.
            [batch_size, N_b, N_k]
        broadcast_atom_to_token_idx (bool): Whether to broadcast atom-to-token
            indices across the batch dimension. Defaults to False.

    Returns:
        torch.Tensor: Gathered pair embeddings.
            [batch_size, N_b, N_q, N_k, d]
            where y[b, nb, i, j, :] = x[b, idx_q[b, nb, i], idx_k[b, nb, j], :]
    """
    idx_q = idx_q.long()
    idx_k = idx_k.long()

    # If doing single example, unsqueeze the idx tensors
    if x.ndim == 4 and idx_q.ndim == idx_k.ndim == 2:
        idx_q = idx_q.unsqueeze(dim=0)
        idx_k = idx_k.unsqueeze(dim=0)

        if broadcast_atom_to_token_idx:
            idx_q = idx_q.repeat(len(x), 1, 1)
            idx_k = idx_k.repeat(len(x), 1, 1)

    # Assert that inputs have the correct number of dimensions
    assert x.ndim == 4
    assert idx_q.ndim == idx_k.ndim == 3
    # Assert that the batch dimension matches across all inputs
    assert x.shape[0] == idx_q.shape[0] == idx_k.shape[0]

    # Get the shape parameters
    batch_size, N_b, N_q = idx_q.shape
    N_k = idx_k.shape[2]

    # 1. Create an explicit index for the batch dimension
    # Shape: [batch_size]
    batch_idx = torch.arange(batch_size, device=x.device)

    # 2. Reshape for broadcasting with other indices.
    # Shape becomes [batch_size, 1, 1, 1]
    batch_idx = batch_idx.view(batch_size, 1, 1, 1)

    # 3. Expand idx_q and idx_k, accounting for the leading batch_size dim
    # idx_q: [B, Nb, Nq] -> [B, Nb, Nq, 1] -> [B, Nb, Nq, Nk]
    idx_q_expanded = idx_q.unsqueeze(-1).expand(-1, -1, -1, N_k)
    # idx_k: [B, Nb, Nk] -> [B, Nb, 1, Nk] -> [B, Nb, Nq, Nk]
    idx_k_expanded = idx_k.unsqueeze(-2).expand(-1, -1, N_q, -1)

    # 4. Use advanced indexing with the explicit batch_idx.
    # PyTorch will broadcast batch_idx to match the other index tensors' shape.
    # This gathers from x[b, row, col] for each batch element `b`.
    y = x[batch_idx, idx_q_expanded, idx_k_expanded]
...
def broadcast_token_to_local_atom_pair(
    ...
    """Broadcasts token-level pair embeddings to local atom-level pair embeddings.

    Maps token pair embeddings to atom pairs using atom-to-token index mapping,
    then rearranges into dense trunk windows for local attention.

    Args:
        z_token (torch.Tensor): Token pair embedding.
            [..., N_token, N_token, d]
        atom_to_token_idx (torch.Tensor): Mapping from atom indices to token indices.
            [N_atom]
        n_queries (int): Local window size of query tensor.
        n_keys (int): Local window size of key tensor.
        compute_mask (bool): Whether to compute the padding mask. Defaults to True.
        broadcast_atom_to_token_idx (bool): Whether to broadcast atom-to-token
            indices across the batch. Defaults to False.

    Returns:
        tuple[torch.Tensor, dict]: A tuple of:
            - z_gathered_blocked: Atom pair embedding in local blocked shape.
                [..., n_trunks, n_queries, n_keys, d]
            - pad_info: Padding information dict from rearrange_qk_to_dense_trunk.
    """
    ...
    z_gathered_blocked = gather_pair_embedding_in_dense_trunk(
        z_token,
        idx_q=atom_to_token_idx_q,
        idx_k=atom_to_token_idx_k,
        broadcast_atom_to_token_idx=broadcast_atom_to_token_idx,
    )
