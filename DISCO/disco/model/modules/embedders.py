...
class InputFeatureEmbedder(nn.Module):
    ...
    def __init__(
        ...
        # Line2
        n_residues = len(STD_RESIDUES) + 1  # Plus one for gap
        self.input_feature = {
            "restype": n_residues,
            "profile": n_residues,
            "deletion_mean": 1,
    ...
    def forward(
        ...
        s_inputs = torch.cat(
            [a]
            + [
                input_feature_dict[name].reshape(*batch_shape, d).to(a.device)
    ...
    def forward(self, input_feature_dict: dict[str, Any]) -> torch.Tensor:
        ...
        Returns:
            torch.Tensor: relative position encoding
                [..., N_token, N_token, c_z]
        """
        device = next(self.linear_no_bias.parameters()).device
        feature_dict = [
            "asym_id",
            "residue_index",
            "entity_id",
        ...
        for feat in feature_dict:
            input_feature_dict[feat] = input_feature_dict[feat].to(device)
        b_same_chain = (
            (
        ...
        a_rel_pos = F.one_hot(d_residue, 2 * (self.r_max + 1)).to(device)
        d_token = torch.clip(
        ...
        a_rel_token = F.one_hot(d_token, 2 * (self.r_max + 1)).to(device)
        d_chain = torch.clip(
