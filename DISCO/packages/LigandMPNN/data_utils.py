...
def parse_PDB(
    ...
    # @NOTE: changed selecting protein_atoms - so masked seq in presence of unmasked seq won't be removed
    protein_atoms = None  # atoms.select("protein")

    if protein_atoms is None:  # from generated unknown sequences
        protein_atoms = atoms.select("name N CA C O")
        backbone = atoms.select("name N CA C O")
    else:
...
def vmappable_get_nearest_neighbours(CB, mask, Y, Y_t, Y_m, number_of_ligand_atoms):
    device = CB.device
    ...
    Y = torch.gather(Y_r, 1, nn_idx[:, :, None].repeat(1, 1, 3))
    Y_t = torch.gather(Y_t_r, 1, nn_idx)
    Y_m = torch.gather(Y_m_r, 1, nn_idx)
...
def featurize(
    ...
    is_cogen_train_loop=False,
):
    output_dict = {}
    nn_method = (
        vmappable_get_nearest_neighbours
        if is_cogen_train_loop
        else get_nearest_neighbours
    )
    if model_type == "ligand_mpnn":
        ...
        CB = -0.58273431 * a + 0.56802827 * b - 0.54067466 * c + CA
        Y, Y_t, Y_m, D_XY = nn_method(CB, mask, Y, Y_t, Y_m, number_of_ligand_atoms)
    ...
    R_idx = input_dict["R_idx"]

    # Count cumulative occurrences of each value
    mask = torch.cat(
        [torch.tensor([False], device=R_idx.device), R_idx[1:] == R_idx[:-1]]
    )
    count = torch.cumsum(mask.long(), dim=0)
    R_idx_renumbered = R_idx + count
